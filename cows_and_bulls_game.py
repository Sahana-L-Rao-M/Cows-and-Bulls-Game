...
def click2():
    ...
    l8=Label(root1,bg="light blue",fg="black",bd=5,text="Player Name : "+e1.get()+" "+e2.get(),font=15)
    l8.pack()
    choice1=Label(root1,bg="light blue",fg="black",bd=5,text="Please enter 4 distinct number in the below box:",font=15)
    choice1.pack()
    choice=Entry(root1,width=20)
    choice.pack()
    scroll_bar = Scrollbar(root1)
    scroll_bar.pack( side = RIGHT,fill = Y )
    mylist = Listbox(root1,borderwidth=0, highlightthickness=0,bg="light pink",width=50,height=15,fg="white",font=("Times",14,'bold'), yscrollcommand = scroll_bar.set )
    mylist.pack()
    ...
    def clicked():
        ...
        s=str(number[2])
        t=str(number[3])
        choice100=Label(hello,bg='light pink',text=q+r+s+t,font=12)
        choice100.pack()
        bint=Label(hello,bg="light blue",fg="blue4",text="Better Luck next time",font=("Helvetica",20,'bold'))
        bint.pack()
    def click():
        global cows
        global bulls
        ...
        if len(choice.get())>len(set(choice.get())):
            ...
            def PlayGame():
                ...
                if cows!=4:
                    ...
                    hello=Tk()
                    hello.state('zoomed')
                    hello.title("COWS AND BULLS")
                    hello.config(background='light pink')
                    l16=Label(hello,text="You WON!\n\n\nAttempts : "+str(attempts),bg="light pink",fg="blue4",font =("Helvetica",24,"bold"))
                    l16.pack()
            PlayGame()
            cows=0
            bulls=0
    choice2=Label(root1,bg="light blue",fg="red",bd=5,text="",font=15)
    choice2.pack()
    b=Button(root1,text="Check",command=click,bg="light grey",fg="black",font=("bold",15))
    b.pack()
    bn=Button(root1,text="Quit",command=clicked,bg="light grey",fg="black",font=15)
    bn.pack()
def click1():
    root=Tk()
    root.title("COWS AND BULLS")
